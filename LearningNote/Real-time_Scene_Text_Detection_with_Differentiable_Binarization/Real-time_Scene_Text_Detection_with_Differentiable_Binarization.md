...
依据上述过程，我们在$G$区域的基础上，以$D$作为偏移量扩展分割区域为$G_d$，$G_s$和$G_d$的间隔被认为是文本区域的边框，可以通过计算到$G$中最近的分割片段的距离来计算阈值图的标签，阈值图中最大阈值`thresh_max`，其他区域均设置为`thresh_min`。

最终概率图和阈值图的标签如下：

attention：基于$Figure{\ }5$进行描述，蓝线以内范围为$G_s$，绿线以内范围为$G_d$

|                 | 蓝线以内区域 |                     绿线和蓝线中间区域                     |   其他区域   |
| :-------------: | :----------: | :--------------------------------------------------------: | :----------: |
|  threshold map  | `thresh_min` | 越靠近红线越接近`thresh_max`，越远离红线越接近`thresh_min` | `thresh_min` |
| probability map |      1       |                             0                              |      0       |
|   binary map    |      1       |                             0                              |      0       |

我对于这样设置的用意，看法是，threshold map主要是为了界定文本边界，probability map是为了界定文本区域
...
对于$L_s$和$L_b$，我们则是使用了二值交叉熵（BCE）计算损失率，此外为了克服正负样本不平均的问题，在BCE中通过采样难负样本进行样本挖掘。
$$
L_s=L_b=\sum_{i{\in}S_i}{y_i}{logx_i}+(1-y_i)log(1-x_i)
$$
$S_l$：采样数据集，其内正负样本比为$1:3$

$L_t$：扩大后文本多边形$G_d$内各像素预测值和标签的$L1$距离的和
$$
